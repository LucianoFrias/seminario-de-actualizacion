...
CREATE PROCEDURE `createUserSession`(IN `id_user` tinyint, IN `token` varchar(256))
BEGIN

DECLARE id_user INT DEFAULT 0;
INSERT INTO user_session(token, created, expires, id_user) VALUES(token, NOW(), DATE_ADD(NOW(), INTERVAL 10 DAY), id_user);
